...
public static partial class Test_Index_Query
{
    private static void QueryArg1 (IndexContext cx)
    {
        ...
        var query5  = store.Query<Position>().                  ValueInRange<IndexedInt, int>(100, 1000);
        cx.query1 = query1;
        cx.query2 = query2;
        cx.query3 = query3;
        cx.query4 = query4;
        cx.query5 = query5;
        {
            ...
            var count = 0;
            query5.ForEachEntity((ref Position pos, Entity entity) => {
                switch (count++) {
                    case 0: AreEqual(12, entity.Id); AreEqual(12f, pos.x); break;
                    case 1: AreEqual(13, entity.Id); AreEqual(13f, pos.x); break;
            ...
            var count = 0;
            foreach (var (positions, entities) in query5.Chunks) {
