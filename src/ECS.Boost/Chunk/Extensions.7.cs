...
{
    public static void Each<TEach, T1,T2,T3,T4,T5,T6,T7>(this Chunks<T1,T2,T3,T4,T5,T6,T7> chunks, ref TEach each)
        ...
        where T7 : struct
    {
        ...
        var span7       = new Span<T7>(chunks.Chunk7.ArchetypeComponents, start, length);
...
#pragma warning restore CS8500
            {
                for (int i = 0; i < length; i++) {
                    ...
                    each.Execute(ref c1[i], ref c2[i], ref c3[i], ref c4[i], ref c5[i], ref c6[i], ref c7[i]);
                }
            }
        }
    }
    
    public static void EachEntity<TEachEntity, T1,T2,T3,T4,T5,T6,T7>(this Chunks<T1,T2,T3,T4,T5,T6,T7> chunks, ref TEachEntity each)
...
#pragma warning restore CS8500
            fixed (int* ids = spanIds)
            {
                for (int i = 0; i < length; i++) {
                    // all spanX.Length == length. spanX[i] is always in bounds.  
                    each.Execute(ref c1[i], ref c2[i], ref c3[i], ref c4[i], ref c5[i], ref c6[i], ref c7[i], ids[i]);
                }
            }
        }
    }
