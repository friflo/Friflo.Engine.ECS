// <auto-generated/>
using System;
using System.ComponentModel;
using System.Runtime.CompilerServices;
using System.Runtime.Intrinsics;
using System.Runtime.Intrinsics.X86;
using Friflo.Vectorization.Intrinsics;
using Friflo.Engine.ECS;

namespace Tests.Generators.Vectorize
{
    public partial class Test_Float_Methods_Avx
    {
        /// <summary>Query method generated for: <see cref="Float_Log"/>.</summary>
        /// <returns>The executed <see cref="ArchetypeQuery"/> for debugging purposes</returns>
        public static ArchetypeQuery Float_LogQuery(EntityStore _store, float value, bool vectorized = true)
        {
            var _query = _Float_Log_GetQuery(_store);
            foreach (var chunk in _query.Chunks)
            {
                var _entities = chunk.Entities;
                var positionSpan = chunk.Chunk1.Span;
                var velocitySpan = chunk.Chunk2.Span;
                int n = 0;
                if (!vectorized) goto EntityLoop;
                if (Avx.IsSupported) {
                    n = _Float_Log_Avx(positionSpan, velocitySpan, value);
                }
            EntityLoop:
                for (; n < _entities.Length; n++) {
                    Float_Log(ref positionSpan[n], in velocitySpan[n], value);
                }
            }
            return _query;
        }

    #region private members
        [EditorBrowsable(EditorBrowsableState.Never)]
        private static readonly int _Float_Log_Slot = EntityStore.UserDataNewSlot();

        [EditorBrowsable(EditorBrowsableState.Never)]
        private static ArchetypeQuery<global::Tests.ECS.Position1, global::Tests.ECS.Velocity1>
            _Float_Log_GetQuery(EntityStore _store)
        {
            var _query = (ArchetypeQuery<global::Tests.ECS.Position1, global::Tests.ECS.Velocity1>)
                EntityStore.UserDataGet(_store, _Float_Log_Slot);
            if (_query != null) {
                return _query;
            }
            _query = _store.Query<global::Tests.ECS.Position1, global::Tests.ECS.Velocity1>();

            EntityStore.UserDataSet(_store, _Float_Log_Slot, _query);
            return _query;
        }

        [SkipLocalsInit]
        private static unsafe int _Float_Log_Avx(
            Span<global::Tests.ECS.Position1> position,
            Span<global::Tests.ECS.Velocity1> velocity,
            float value)
        {
            int i = 0;
            var end = position.Length - 32;
            if (i > end) {
                return 0;
            }
            // Vector layout: AoS
            // --- Locals
            var value_scalar = Vector256.Create(value);

            var const0 = Vector256.Create(0x7FFFFFFF).AsSingle(); // Abs()

            fixed (global::Tests.ECS.Position1* position_first = position)
            fixed (global::Tests.ECS.Velocity1* velocity_first = velocity)
            {
                for (; i <= end; i += 32)
                {
                    float* position_ptr = (float*)(position_first + i);
                    float* velocity_ptr = (float*)(velocity_first + i);

                    // --- 1. Load
                    Vector256<float> position_0 = Avx.LoadVector256(position_ptr +  0);  // Position1
                    Vector256<float> position_1 = Avx.LoadVector256(position_ptr +  8);  // Position1
                    Vector256<float> position_2 = Avx.LoadVector256(position_ptr + 16);  // Position1
                    Vector256<float> position_3 = Avx.LoadVector256(position_ptr + 24);  // Position1

                    Vector256<float> velocity_0 = Avx.LoadVector256(velocity_ptr +  0);  // Velocity1
                    Vector256<float> velocity_1 = Avx.LoadVector256(velocity_ptr +  8);  // Velocity1
                    Vector256<float> velocity_2 = Avx.LoadVector256(velocity_ptr + 16);  // Velocity1
                    Vector256<float> velocity_3 = Avx.LoadVector256(velocity_ptr + 24);  // Velocity1

                    // --- 2. Compute
                    // var abs = MathF.Abs(velocity.value);
                    var abs_0 = Avx.And(velocity_0, const0);
                    var abs_1 = Avx.And(velocity_1, const0);
                    var abs_2 = Avx.And(velocity_2, const0);
                    var abs_3 = Avx.And(velocity_3, const0);

                    // var log = MathF.Log(abs);
                    var log_0 = Vector256.Log(abs_0);
                    var log_1 = Vector256.Log(abs_1);
                    var log_2 = Vector256.Log(abs_2);
                    var log_3 = Vector256.Log(abs_3);

                    // position.value = log;
                    position_0 = log_0;
                    position_1 = log_1;
                    position_2 = log_2;
                    position_3 = log_3;

                    // --- 3. Store
                    Avx.Store(position_ptr +  0, position_0);
                    Avx.Store(position_ptr +  8, position_1);
                    Avx.Store(position_ptr + 16, position_2);
                    Avx.Store(position_ptr + 24, position_3);
                }
            }
            return i;
        }

    #endregion
    }
}
