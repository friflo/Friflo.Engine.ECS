    ...
    where T : struct
{
    ...
    internal  override  TField GetComponentField<TField> (int compIndex, MemberPath info) {
        var getter = (MemberPathGetter<T, TField>)info.getter;
        return getter(components[compIndex]);
    }
    
    internal  override  void SetComponentField<TField>(int compIndex, MemberPath info, TField value) {
        var setter = (MemberPathSetter<T, TField>)info.setter;
