...
namespace Tests.Generators.Vectorize
{
    public partial class Test_Float_Avx
    {
        ...
        public static ArchetypeQuery MultiplyVectorQuery(EntityStore _store, float factor, bool vectorized = true)
        {
            ...
            foreach (var chunk in _query.Chunks)
            {
                ...
                if (Avx.IsSupported) {
                    n = _MultiplyVector_Avx(positionSpan, velocitySpan, factor);
                }
            EntityLoop:
                for (; n < _entities.Length; n++) {
                    MultiplyVector(ref positionSpan[n], in velocitySpan[n], factor);
                }
            }
            return _query;
        }
            ...
            float factor)
        {
            ...
            fixed (global::Tests.ECS.Velocity1* velocity_first = velocity)
            {
                for (; i <= end; i += 32)
                {
                    ...
                    position_0 = Avx.Multiply(position_0, Avx.Multiply(velocity_0, factor_scalar));
                    position_1 = Avx.Multiply(position_1, Avx.Multiply(velocity_1, factor_scalar));
                    position_2 = Avx.Multiply(position_2, Avx.Multiply(velocity_2, factor_scalar));
                    position_3 = Avx.Multiply(position_3, Avx.Multiply(velocity_3, factor_scalar));
