...
namespace Tests.Generators.Vectorize
{
    ...
    {
        /// <summary>Query method generated for: <see cref="Multiply_Vector2_static"/>.</summary>
        /// <returns>The executed <see cref="ArchetypeQuery"/> for debugging purposes</returns>
        public static ArchetypeQuery Multiply_Vector2_staticQuery(EntityStore _store, bool vectorized = true)
        {
            var _query = _Multiply_Vector2_static_GetQuery(_store);
            foreach (var chunk in _query.Chunks)
            {
                ...
                if (Avx.IsSupported) {
                    n = _Multiply_Vector2_static_Avx(positionSpan, velocitySpan);
                }
            EntityLoop:
                for (; n < _entities.Length; n++) {
                    Multiply_Vector2_static(ref positionSpan[n], ref velocitySpan[n]);
                }
            }
            return _query;
        }
    ...
    #region private members
        ...
        private static readonly int _Multiply_Vector2_static_Slot = EntityStore.UserDataNewSlot();

        [EditorBrowsable(EditorBrowsableState.Never)]
        private static ArchetypeQuery<global::Tests.ECS.Position2, global::Tests.ECS.Velocity2>
            _Multiply_Vector2_static_GetQuery(EntityStore _store)
        {
            var _query = (ArchetypeQuery<global::Tests.ECS.Position2, global::Tests.ECS.Velocity2>)
                EntityStore.UserDataGet(_store, _Multiply_Vector2_static_Slot);
            if (_query != null) {
                return _query;
            }
            _query = _store.Query<global::Tests.ECS.Position2, global::Tests.ECS.Velocity2>();

            EntityStore.UserDataSet(_store, _Multiply_Vector2_static_Slot, _query);
            return _query;
        }

        [SkipLocalsInit]
        private static unsafe int _Multiply_Vector2_static_Avx(
